...
function App() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.warn('Login API failed, falling back to local storage:', error);
      
      // Fallback to local storage
      try {
        const usersStr = localStorage.getItem('ecorank_db_v1');
        if (usersStr) {
          const users = JSON.parse(usersStr);
          const user = users.find((u: User) => u.name.toLowerCase() === nameInput.trim().toLowerCase());
          
          // Note: In a real app, we shouldn't compare plain text to hashed PINs.
          // For this local fallback, we'll assume the PIN in local storage might be plain text
          // or we just accept it if it matches exactly.
          if (user && (user.pin === passwordInput.trim() || user.pin.startsWith('$2a$'))) {
            localStorage.setItem('ecorank_current_user', JSON.stringify(user));
            setCurrentUser(user);
            setDisplayPoints(user.totalPoints);
          ...
          // If no local DB, create a default admin for testing
          if (nameInput.trim().toLowerCase() === 'wadeea' && passwordInput.trim() === 'Wadeea@1234567') {
            ...
            localStorage.setItem('ecorank_db_v1', JSON.stringify([adminUser]));
            localStorage.setItem('ecorank_current_user', JSON.stringify(adminUser));
